...
// 1. Destructured active and onNavigate from props
function Navbar({ active, onNavigate }) {
  const [mobileOpen, setMobileOpen] = useState(false);

  const handleNav = (id) => {
    // 2. Added a safety check to ensure onNavigate exists before firing
    ...
    <>
     ...
     <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <ul className="nav-links">
          {NAV_ITEMS.map(item => (
            <li key={item.id} className="nav-link-item">
              <button
                // 3. 'active' is now properly defined from props
        ...
        <div className="nav-footer">
          ...
          {/* 4. Swapped text glyph for an SVG to match sizes precisely */}
          <a
